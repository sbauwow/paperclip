...
function resolveProwlerSkillsHome(config: Record<string, unknown>) {
  const env =
...
async function buildProwlerSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  const availableEntries = await readPaperclipRuntimeSkillEntries(config, __moduleDir);
  const availableByKey = new Map(availableEntries.map((entry) => [entry.key, entry]));
  const desiredSkills = resolvePaperclipDesiredSkillNames(config, availableEntries);
  const desiredSet = new Set(desiredSkills);
  const skillsHome = resolveProwlerSkillsHome(config);
  ...
  const entries: AdapterSkillEntry[] = availableEntries.map((entry) => ({
    ...
    detail: desiredSet.has(entry.key)
      ? "Will be exposed to the Prowler/Hermes runtime on the next run."
      : null,
...
export async function listSkills(ctx: AdapterSkillContext): Promise<AdapterSkillSnapshot> {
  return buildProwlerSkillSnapshot(ctx.config);
}
